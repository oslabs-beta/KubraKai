...
const verifyCallBackLocal = async (username, password, done) => {
  const queryStrselect = `select * from localUsers where email_fk = $1`;
  ...
  db.query(selectStr, [profile.emails[0].value])
    .then((currentUser) => {
      if (currentUser.rows[0]) {
        ...
        db.query(insertUserStr, [
          profile.name.givenName,
          profile.name.familyName,
          profile.emails[0].value,
        ])
          .then((newuser) => {
            db.query(insertOauthStr, [
              profile.id,
              profile.provider,
              profile.emails[0].value,
            ])
...
*/
passport.serializeUser((user, done) => done(null, user.email_fk || user.email));
